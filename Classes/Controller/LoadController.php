...
class Tx_MootoolsEssentials_Controller_LoadController extends Tx_Extbase_MVC_Controller_ActionController {
	...
	public function load($settings) {
		...
		foreach ($files as $file) {
			...
			$path = is_array($cssFileArray) && array_key_exists('manifest', $cssFileArray) && array_key_exists('path', $cssFileArray) ? $settings['manifests'][$cssFileArray['manifest']] . $cssFileArray['path'] : '';
			if ($path !== '') {
		...
		if ($settings['language'] !== '' && $this->hasPackage('More/Locale', $files)) {
